...
const SelectBSColors = ({ color = 'none', onChange, label, id }) => {
  ...
  const selectColors = bsColor ? `bg-${bsColor} text-bg-${bsColor}` : '';
  ...
  return (
    <div className="mb-3">
      ...
      <select id={id} className={`form-select ${selectColors}`} onChange={handleChange} value={bsColor}>
...
SelectBSColors.propTypes = {
  color: propTypes.oneOf(['none', 'primary', 'secondary', 'success', 'danger', 'warning', 'info', 'light', 'dark']),
  onChange: propTypes.func,
  label: propTypes.string,
  id: propTypes.string.isRequired,
};

export default SelectBSColors;